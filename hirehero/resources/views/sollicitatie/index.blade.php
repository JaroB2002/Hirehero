  ...
  <body>
        ...
        <main class="h-full pb-16 overflow-y-auto">
          <div class="container px-6 mx-auto grid">
            ...
            >
              Sollicitaties van {{--$sollicitatie->vacature->title--}}
            </h2>
  
            <h2
            class="my-6 text-1xl text-gray-700 dark:text-gray-200"
          >
          Hier kan je al de sollicitaties voor deze vacature bekijken
          </h2>
                  ...
                  <tr class="text-gray-700 dark:text-gray-400" style="background-color: #C4E4F7;">
                    <td class="px-4 py-3">
                        <div class="flex items-center text-sm">   
                          ...
                          <div>
                            <a href="/bedrijf/vacature/{{$sollicitatie->vacature_id}}">
                                <p class="font-semibold">{{$sollicitatie->student->voornaam}}</p>
                            </a>
                    ...
                    <form action="{{'/bedrijf/vacature/' . $id = $sollicitatie->vacature_id . '/sollicitaties'}}" method="POST" class="statusForm ">
                      ...
                      <td class="px-4 py-3 text-sm">
                        {{$sollicitatie->created_at->format('d/m/Y')}}
                      </td>
...
@endforeach                    
              ...
              >
                <span class="flex items-center col-span-3">
                  Je bekijkt {{$sollicitaties->firstItem()}} to {{$sollicitaties->lastItem()}} of {{$sollicitaties->total()}} vacatures
                </span>
                  ...
                  <nav aria-label="Table navigation">
                    <ul class="inline-flex items-center">
                      <li>
                        ...
                        <li>
                            @for ($i = 1; $i <= $sollicitaties->lastPage(); $i++)
                            <button
                            class="px-3 py-1 rounded-md focus:outline-none focus:shadow-outline-purple @if($sollicitaties->currentPage() == $i) bg-purple-500 text-white @endif"
                            >
                            <a href="{{$sollicitaties->url($i)}}">{{$i}}</a>
